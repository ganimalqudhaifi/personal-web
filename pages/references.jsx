...
export default function References() {
  return (
    <PrimaryLayout activeIndex={3}>
      ...
      <div>
        <div className="flex items-center space-x-2 text-zinc-200"><PiBookBookmark size={24}/><h2 className="text-xl tracking-tight">Course</h2></div>
            ...
            courseLists.map((course,i) => (
              <div key={i} className="flex justify-between items-center p-2 bg-zinc-600/5 border border-zinc-800/60 rounded shadow">
                <div className="flex space-x-2 items-center">
                  <Image alt={`${course.name} logo`} src={course.src} width={75} height={75} className="w-12 rounded shadow"/>
                  <p className="tracking-tight">{course.name}</p>
                </div>
